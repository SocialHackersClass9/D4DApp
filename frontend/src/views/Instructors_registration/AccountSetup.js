...
class AccountSetup extends Component {
  ...
  request() {
    axios.get('http://localhost:3001/api/Instructors_registration/used', { 'headers': { 'key': 'd4dapplicationregistrationapigetmethod1234567890' } })
      .then(response => {
        var emailuser = response.data.emailuser;
        emailuser.map((i) => {
          this.setState({ ...this.state, emails: [...this.state.emails, i.email] })
          this.setState({ ...this.state, user_names: [...this.state.user_names, i.user_name] })
        })
      })
      .catch(err => {
